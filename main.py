...
class Task_Manager(MDApp):
    def build(self):
        self.theme_cls.theme_style = self.load_theme()  # Load saved theme
        ...
        menu_list = [
            {"text": "Light Mode", "on_release": lambda x="Light": self.set_theme(x)},
            {"text": "Dark Mode", "on_release": lambda x="Dark": self.set_theme(x)},
        ]
        ...
        settings_button.bind(on_release=lambda x: dropdown_menu.open())
    ...
    # Show confirmation dialog
        self.dialog = MDDialog(
            title="Congrats!",
            text=f" You completed: {task_text}",
            buttons=[
                MDFlatButton(
                    text="OK",
                    md_bg_color=[101 / 255, 71 / 255, 1, 1],
